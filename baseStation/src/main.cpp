...
int speedMonitoring(CoreAPI* api, Flight* flight, float32_t velocity_threshold, float32_t height_threshold)
{
    while(!endFlag)
    {
        usleep(100000);
        ...
        if (curVelocity.x > velocity_threshold || curVelocity.x > velocity_threshold || curVelocity.x > velocity_threshold)
        {
            std::cout << "Velocity exceeded threshold ! Emergency release of the control." << std::endl;
        ...
        {
            std::cout << "Height exceeded threshold ! Emergency release of the control." << std::endl;
            ackReturnData releaseControlStatus = releaseControl(api);
            endFlag = 1;
        } 
    }   
...
void turnMain(CoreAPI* api, Flight* flight)
{
    ...
    while(turnFlag)
    {
        int status1 = moveWithVelocity(api, flight, 0, 0, 0, angularSpeed, 1500, 3, 0.1);
    }
...
int main(int argc, char *argv[])
{
    ...
    //std::thread monitoringThread(speedMonitoring, api, flight, v_threshold, height_threshold);
    ...
    else if (argv[1] && !strcmp(argv[1], "-programmatic-radio1"))
    {
        ...
        if (takeoffStatus.status == 1)
        {
            ...
            //{   
                ...
                //int status1 = moveWithVelocity(api, flight, 0, 0, 0, angularSpeed*2, 10, 3, 0.1);
                ...
                //}
                //std::cout << "Offset - RadioValue : " << offset << " - " << currentRadioVal << std::endl;
                //lastYaw = currentYaw*RAD2DEG;

                ////usleep(5000);
            //}

            std::cout << "Tour is done, best yaw was " << bestYaw*RAD2DEG << " with a radio value of " << bestRadioVal << std::endl;
			bestYaw = 0;
            // Head in the computed direction
            int status1 = moveByPositionOffset(api,flight, radialDist*cos(bestYaw),radialDist*sin(bestYaw),0,bestYaw,15000000,3,30);
    ...
    else if (argv[1] && !strcmp(argv[1], "-programmatic-radio2"))
    {
        ...
        if (takeoffStatus.status == 1)
        {
            ...
            // Need to do the "turn" command in a separate thread because it is blocking
            turnFlag = 1;
            ...
            {   
                ...
                offset += std::min(abs(currentYaw - lastYaw),std::min(abs(360-lastYaw + currentYaw),abs(360-currentYaw+lastYaw)));
                currentRadioVal = measureFromRadio;
                currentSwitchState = switchState;
                currentFreq = current_freq;

                if (currentRadioVal > bestRadioVal[currentFreq] && currentRadioVal < 10)
                {
            ...
            std::cout << "bestRadioVal :" << bestRadioVal[0] << ", " << bestRadioVal[1] << std::endl;
	        std::cout << "bestYaw :" << bestYaw[0] << ", " << bestYaw[1] << std::endl;
            std::cout << "Frequency 0: best yaw was " << bestYaw[0] << " with a radio value of " << bestRadioVal[0] << std::endl;
            std::cout << "The corresponding switch state was " << bestSwitchState[0] << std::endl;

            std::cout << "Frequency 1: best yaw was " << bestYaw[1] << " with a radio value of " << bestRadioVal[1] << std::endl;
            std::cout << "The corresponding switch state was " << bestSwitchState[1] << std::endl;

            weakestFreqIndex = std::distance(bestRadioVal.begin(), std::min_element(bestRadioVal.begin(), bestRadioVal.end()));
            ...
            // Head in the computed direction
            int status1 = moveByPositionOffset(api,flight, radialDist*cos(DEG2RAD*finalYaw),radialDist*sin(DEG2RAD*finalYaw),0,finalYaw,15000,3,30);
	...
	for(int i = 0; i < measurementsF1.size(); ++i)
	{
        ...
        std::cout << std::endl << "Here is the content of bestMeasurementF1:" << std::endl;
        std::cout << bestMeasurementF1[0] << " - " << bestMeasurementF1[1] << " - " << bestMeasurementF1[2] << std::endl; 

        std::cout << std::endl << "Here is the content of bestMeasurement2F1:" << std::endl;
        std::cout << bestMeasurement2F1[0] << " - " << bestMeasurement2F1[1] << " - " << bestMeasurement2F1[2] << std::endl << std::endl;

        std::cout << std::endl << "Here is the content of bestMeasurementF2:" << std::endl;
        std::cout << bestMeasurementF2[0] << " - " << bestMeasurementF2[1] << " - " << bestMeasurementF2[2] << std::endl; 

        std::cout << std::endl << "Here is the content of bestMeasurement2F2:" << std::endl;
        std::cout << bestMeasurement2F2[0] << " - " << bestMeasurement2F2[1] << " - " << bestMeasurement2F2[2] << std::endl << std::endl;
     ...
     //    std::cout << std::endl << "The final yaw in simple case is " << finalYaw1 << std::endl;
        ...
        {
            finalYaw2 = int(bestMeasurementF2[1] + (bestMeasurementF2[0]*offsets[bestMeasurementF2[2]] 
                + bestMeasurement2F2[0]*offsets[bestMeasurement2F2[2]])
                /(bestMeasurementF2[0]+bestMeasurement2F2[0])) % 360;
            std::cout << "F2 was weaker ! Going for yaw " << finalYaw2 << std::endl;
        }
        else
        {
            finalYaw2 = int(bestMeasurementF1[1] + (bestMeasurementF1[0]*offsets[bestMeasurementF1[2]] 
                + bestMeasurement2F1[0]*offsets[bestMeasurement2F1[2]])
                /(bestMeasurementF1[0]+bestMeasurement2F1[0])) % 360;
        ...
        char write;
        std::cout << "Want to save this decision point to file ? (y for yes)" << std::endl;
        std::cin >> write;
        if (write == 'y')
        {
	    ...
	    outfile.precision(16);
            ...
            PositionData pos = flight -> getPosition();
            outfile << bestMeasurementF1[0]       << ","
                    << bestMeasurementF1[1]       << ","
                    << bestMeasurementF1[2]       << ","
                    << bestMeasurement2F1[0]      << ","
                    << bestMeasurement2F1[1]      << ","
                    << bestMeasurement2F1[2]      << ","
		    << bestMeasurementF2[0]       << ","
		<< bestMeasurementF2[1] << ","
		<< bestMeasurementF2[2] << ","
		<< bestMeasurement2F2[0] << ","
		<< bestMeasurement2F2[1] << ","
		<< bestMeasurement2F2[2] << "," 
                    << finalYaw2                << ","
                    << pos.latitude             << ","
